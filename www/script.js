...
function setupEventListeners() {
    ...
    menuFromOverlayBtn.addEventListener('click', returnToMenu);
...
function handleKeyDown(e) {
    ...
    if (e.key === 'Escape' && state.gameRunning) pauseGame();
}
...
function showOverlay(title, subtitle, showNameInput = false) {
    ...
    if (state.isPaused) {
        ...
        document.getElementById('levelSelectFromOverlayBtn').addEventListener('click', showLevelSelect);
        document.getElementById('menuFromOverlayBtn').addEventListener('click', returnToMenu);
    } else {
        overlayButtons.innerHTML = `
            <button type="button" class="btn btn-primary" id="restartBtn">Restart Level</button>
            <button type="button" class="btn btn-secondary" id="menuFromOverlayBtn">Main Menu</button>
        `;
    }
...
function saveHighScore() {
    ...
    let name = 'Player';
    if (nameInputContainer.style.display !== 'none' && playerNameInput.value.trim()) {
...
function updateHighScores(mode) {
    ...
    if (!highScores[mode] || highScores[mode].length === 0) {
        scoresList.innerHTML = '<div class="score-item"><div>No scores yet!</div></div>';
        return;
    }
...
function clearHighScores() {
    if (confirm('Are you sure you want to clear all high scores?')) {
        const activeTab = document.querySelector('.score-tab.active').dataset.mode;
        try {
            const scoresData = localStorage.getItem('blockbastHighScores') || '{}';
            const highScores = JSON.parse(scoresData);
            highScores[activeTab] = [];
            localStorage.setItem('blockbastHighScores', JSON.stringify(highScores));
            updateHighScores(activeTab);
        } catch (e) {
